...
//TEST PATH STRING  "/home/USER/elrond-wasm-rs/contracts/feature-tests/basic-features/mandos"
use std::{fs, env};
...
fn read_dirs(path: &str) -> Vec<String>{
    ...
    for dir in paths{
        let dir_abs_path = String::from(dir.unwrap().path().into_os_string().into_string().unwrap());
