	...
	match &arg.ty {
		syn::Type::Reference(type_reference) => {
			...
			if let syn::Type::Slice(slice_type) = &*type_reference.elem {
				...
				quote! {
					elrond_wasm::load_single_arg::<T, Box<[#slice_elem]>>(self.api.clone(), #arg_index_expr, #arg_name_expr)
				}
			} else {
				...
				quote! {
					elrond_wasm::load_single_arg::<T, #referenced_type>(self.api.clone(), #arg_index_expr, #arg_name_expr)
				}
			}
		},
		_ => {
			quote! {
				elrond_wasm::load_single_arg::<T, #arg_ty>(self.api.clone(), #arg_index_expr, #arg_name_expr)
			}
		},
	}
}
