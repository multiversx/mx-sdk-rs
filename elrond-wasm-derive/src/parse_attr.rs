...
fn find_attr_one_string_arg(m: &syn::TraitItemMethod, attr_name: &str) -> Option<String> {
	let event_attr = m
		.attrs
		.iter()
		.find(|attr| if let Some(first_seg) = attr.path.segments.first() { first_seg.ident == attr_name } else { false });
	match event_attr {
		None => None,
		Some(attr) => {
...
impl StorageGetAttribute {
	pub fn parse(m: &syn::TraitItemMethod) -> Option<StorageGetAttribute> {
		match find_attr_one_string_arg(m, ATTR_STORAGE_GET) {
			None => None,
			Some(arg_str) => Some(StorageGetAttribute { identifier: arg_str }),
		}
	}
}
...
impl StorageSetAttribute {
	pub fn parse(m: &syn::TraitItemMethod) -> Option<StorageSetAttribute> {
		match find_attr_one_string_arg(m, ATTR_STORAGE_SET) {
			None => None,
			Some(arg_str) => Some(StorageSetAttribute { identifier: arg_str }),
		}
	}
}
...
impl EndpointAttribute {
	pub fn parse(m: &syn::TraitItemMethod) -> Option<EndpointAttribute> {
		match find_attr_with_one_opt_token_tree_arg(m, ATTR_ENDPOINT) {
			None => None,
			Some(Some(proc_macro2::TokenTree::Ident(ident))) => Some(EndpointAttribute { endpoint_name: Some(ident) }),
			Some(None) => Some(EndpointAttribute { endpoint_name: None }),
...
impl ViewAttribute {
	pub fn parse(m: &syn::TraitItemMethod) -> Option<ViewAttribute> {
		match find_attr_with_one_opt_token_tree_arg(m, ATTR_VIEW) {
			None => None,
			Some(Some(proc_macro2::TokenTree::Ident(ident))) => Some(ViewAttribute { view_name: Some(ident) }),
...
impl CallbackCallAttribute {
	pub fn parse(m: &syn::TraitItemMethod) -> Option<CallbackCallAttribute> {
		match find_attr_with_one_opt_token_tree_arg(m, ATTR_CALLBACK_CALL) {
			None => None,
			Some(Some(proc_macro2::TokenTree::Ident(ident))) => Some(CallbackCallAttribute { arg: ident }),
...
impl MultiAttribute {
	pub fn parse(pat: &syn::PatType) -> Option<MultiAttribute> {
		let multi_attr = pat
			.attrs
			.iter()
			.find(|attr| if let Some(first_seg) = attr.path.segments.first() { first_seg.ident == ATTR_MULTI } else { false });

		match multi_attr {
			None => None,
			Some(attr) => {
				let mut iter = attr.clone().tokens.into_iter();
				let count_expr: proc_macro2::TokenStream = match iter.next() {
