...
pub fn arg_serialize_push(
    ...
    match arg_ty {
        syn::Type::Path(_) => {
            ...
            // arg_serialize_push_single(&type_path_segment, &arg_accumulator, &var_name)
            quote!{
                AsynCallArg::push_async_arg(&#var_name, &mut #arg_accumulator);
            }
        },
        syn::Type::Reference(type_reference) => {
            if type_reference.mutability.is_some() {
...
pub fn arg_serialize_push_multi(
    arg: &MethodArg,
    arg_accumulator: &proc_macro2::TokenStream,
    expected_count_expr: &proc_macro2::TokenStream) -> proc_macro2::TokenStream {
    ...
    match arg_ty {
        syn::Type::Path(_) => {
            quote!{
                if let Err(sc_err) = AsynCallArg::push_async_arg_exact(&#var_name, &mut #arg_accumulator, #expected_count_expr) {
...
pub fn arg_serialize_push_multi_old(
        arg: &MethodArg,
        arg_accumulator: &proc_macro2::TokenStream,
        var_name: &proc_macro2::TokenStream) -> proc_macro2::TokenStream {
    match &arg.ty {
