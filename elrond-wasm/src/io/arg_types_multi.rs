...
where
    ...
    E: DynArgErrHandler,
{
    match T::load_exact(loader, arg_id, num) {
        Ok(arg) => arg,
...
pub trait ArgTypeMulti<D>: ArgType<D> {
    fn load_exact(_loader: &mut D, _arg_name: ArgId, _num: usize) -> Result<Self, SCError>;
}
...
where
    ...
    D: DynArgLoader<()>,
{
    fn load_exact(loader: &mut D, arg_id: ArgId, num: usize) -> Result<Self, SCError> {
