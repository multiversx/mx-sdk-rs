...
mod tests {
    use super::*;
    // use crate::serializer::{to_bytes, from_bytes};

    #[test]
    fn test_next_raw_bytes_1() {
        //let cd = CallDataSerializer::from_raw_data(().to_vec());
        let input: &[u8] = b"func@1111@2222";
        let mut de = CallDataDeserializer::new(input);
        assert_eq!(de.next_argument_hex(), Some(&b"func"[..]));
        assert_eq!(de.next_argument_hex(), Some(&b"1111"[..]));
        assert_eq!(de.next_argument_hex(), Some(&b"2222"[..]));
        assert_eq!(de.next_argument_hex(), None);
        assert_eq!(de.next_argument_hex(), None);
    }
    ...
    fn test_next_raw_bytes_empty() {
        ...
        assert_eq!(de.next_argument_hex(), Some(&[][..]));
        assert_eq!(de.next_argument_hex(), None);
    }
    ...
    fn test_next_raw_bytes_all_empty_but_last() {
        ...
        assert_eq!(de.next_argument_hex(), Some(&[][..]));
        assert_eq!(de.next_argument_hex(), Some(&[][..]));
        assert_eq!(de.next_argument_hex(), Some(&b"123"[..]));
        assert_eq!(de.next_argument_hex(), None);
        assert_eq!(de.next_argument_hex(), None);
    }
    ...
    // #[test]
    // fn test_next_h256() {
    //     let cd = CallDataSerializer::from_raw_data((&b"func@0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef").to_vec());
    ...
    //                              ];
    //     assert_eq!(de.next_argument_hex(), Some(&b"func"[..]));
    //     assert_eq!(de.next_vec_u8(), Some(expected[..].to_vec()));
    //     assert_eq!(de.next_argument_hex(), None);
    //     assert_eq!(de.next_argument_hex(), None);
    // }
    ...
    //     let cd = CallDataSerializer::from_raw_data((&b"func@123").to_vec());
    //     let mut de = cd.deserializer();
    //     assert_eq!(de.next_argument_hex(), Some(&b"func"[..]));
