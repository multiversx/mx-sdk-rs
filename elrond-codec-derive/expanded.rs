...
impl NestedDecode for DayOfWeek {
    fn dep_decode<I: NestedDecodeInput>(input: &mut I) -> Result<Self, DecodeError> {
        let return_value = match u8::dep_decode(input)? {
            0u8 => Some(DayOfWeek::Monday),
            1u8 => Some(DayOfWeek::Tuesday),
            2u8 => Some(DayOfWeek::Wednesday),
            3u8 => Some(DayOfWeek::Thursday),
            4u8 => Some(DayOfWeek::Friday),
            5u8 => Some(DayOfWeek::Saturday),
            6u8 => Some(DayOfWeek::Sunday),
            _ => None,
        };
        match return_value {
            Some(r) => Ok(r),
...
impl NestedDecode for Message {
    fn dep_decode<I: NestedDecodeInput>(input: &mut I) -> Result<Self, DecodeError> {
        let return_value = match u8::dep_decode(input)? {
            0u8 => Some(Message::Quit),
            1u8 => Some(Message::Today(<DayOfWeek>::dep_decode(input)?)),
            2u8 => Some(Message::Write(<Vec<u8>>::dep_decode(input)?)),
            _ => None,
        };
        match return_value {
            Some(r) => Ok(r),
    ...
    fn dep_decode_or_exit<I: NestedDecodeInput, ExitCtx: Clone>(
        ...
        let return_value = match u8::dep_decode_or_exit(input, c.clone(), exit) {
            0u8 => Some(Message::Quit),
            1u8 => Some(Message::Today(<DayOfWeek>::dep_decode_or_exit(
                input,
                c.clone(),
                exit,
            ))),
            2u8 => Some(Message::Write(<Vec<u8>>::dep_decode_or_exit(
                input,
                c.clone(),
                exit,
            ))),
