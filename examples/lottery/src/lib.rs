...
pub trait Lottery {
	...
	fn start_lottery(
		...
		require!(total_tickets <= MAX_TICKETS, "Only 800 or less total tickets per lottery are allowed!");
			...
			for i in (0..for_loop_end).rev() {
				...
				loop {
					...
					if !prev_winning_tickets.contains(&winning_ticket_id) {
						...
						if i != 0 {
							prize = BigUint::from(info.prize_distribution[i] as u32) * info.prize_pool.clone() / BigUint::from(PERCENTAGE_TOTAL as u32);
						} else {
