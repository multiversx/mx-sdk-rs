...
fn test_big_uint_format() {
    ...
    assert_eq!("BigUint { handle: -10, hex-value-be: \"1234\" }", s);
}
...
fn test_big_int_format_1() {
    ...
    assert_eq!("BigInt { handle: -10, hex-value-be: \"1234\" }", s);
}
...
fn test_big_int_format_2() {
    let _ = DebugApi::dummy();
    let s = format!("{:?}", BigInt::<DebugApi>::from(-0x1234));
    assert_eq!("BigInt { handle: -10, hex-value-be: \"edcc\" }", s);
}
...
fn test_managed_buffer() {
    ...
    assert_eq!("ManagedBuffer { handle: -10, hex-value: \"1234\" }", s);
}
...
fn test_managed_byte_array() {
    ...
    assert_eq!(
        "ManagedByteArray { handle: -10, size: 7, hex-value: \"01020304050607\" }",
        s
    );
}
...
fn test_managed_address() {
    ...
    assert_eq!("ManagedAddress { handle: -10, hex-value: \"000000000000000000010000000000000000000000000000000000000002ffff\" }", s);
}
...
fn test_managed_vec_format() {
    ...
    let s = format!("{:?}", &mv);
    assert_eq!("[BigUint { handle: -10, hex-value-be: \"01\" }, BigUint { handle: -11, hex-value-be: \"02\" }]", s);
}
