...
impl TxContext {
    pub fn new(tx_input: TxInput, blockchain_cell: Rc<RefCell<BlockchainMock>>) -> Self {
        TxContext {
            ...
            blockchain_cache: TxCache::new(blockchain_cell),
    ...
    pub fn dummy() -> Self {
        TxContext {
            ...
            blockchain_cache: TxCache::new(Rc::new(RefCell::new(BlockchainMock::new()))),
            managed_types: RefCell::new(TxManagedTypes::new()),
            tx_result_cell: RefCell::new(TxResult::empty()),
        }
    }
    ...
    pub fn create_new_contract(&self, contract_path: Vec<u8>, contract_owner: Address) -> Address {
        let sender_nonce_before_tx = self.get_sender_account().nonce - 1;
