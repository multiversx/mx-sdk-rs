...
use kitty::{Kitty, KittyGenes, Color};
use random::Random;
...
pub trait KittyGeneticAlg {
	...
	fn generate_kitty_genes(&self, matron: &Kitty, sire: &Kitty) -> KittyGenes {
		...
		let kitty_fur_color = matron_fur_color.mix_with(sire_fur_color, 
			fur_color_percentage, 100 - fur_color_percentage);
		...
		let kitty_eye_color = matron_eye_color.mix_with(sire_eye_color, 
			eye_color_percentage, 100 - eye_color_percentage);
