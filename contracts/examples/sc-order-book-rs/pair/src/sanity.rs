...
pub trait SanityCheckModule: common::CommonModule {
    ...
    fn require_valid_match_input_order_ids(&self, order_ids: &Vec<u64>) -> SCResult<()> {
    ...
    fn require_not_max_size(&self, address_order_ids: &Vec<u64>) -> SCResult<()> {
        require!(
    ...
    fn require_order_ids_not_empty(&self, order_ids: &Vec<u64>) -> SCResult<()> {
    ...
    fn require_creator_is_not_caller(&self, orders: &Vec<Order<Self::BigUint>>) -> SCResult<()> {
        let caller = &self.blockchain().get_caller();
    ...
    fn require_contains_all(&self, vec_base: &Vec<u64>, items: &Vec<u64>) -> SCResult<()> {
    ...
    fn require_contains_none(&self, vec_base: &Vec<u64>, items: &Vec<u64>) -> SCResult<()> {
        for item in items.iter() {
