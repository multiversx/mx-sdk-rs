...
{
    #[init]
    fn init(&self, quorum: usize, #[var_args] board: ManagedMultiValue<ManagedAddress>) {
    ...
    fn get_pending_action_full_info(&self) -> ManagedMultiValue<ActionFullInfo<Self::Api>> {
    ...
    fn get_all_board_members(&self) -> ManagedMultiValue<ManagedAddress> {
        self.get_all_users_with_role(UserRole::BoardMember)
    }

    /// Lists all proposers that are not board members.
    #[external_view(getAllProposers)]
    fn get_all_proposers(&self) -> ManagedMultiValue<ManagedAddress> {
        self.get_all_users_with_role(UserRole::Proposer)
    }

    fn get_all_users_with_role(&self, role: UserRole) -> ManagedMultiValue<ManagedAddress> {
        let mut result = ManagedMultiValue::new();
