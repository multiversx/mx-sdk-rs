...
const ADDER_CODE_PATH: MxscPath = MxscPath::new("../output/returns-token-identifier-test.mxsc.json");

#[tokio::main]
async fn main() {
    env_logger::init();

    let mut basic_interact = ReturnsTokenIdentifierTestInteract::init().await;
...
#[allow(unused)]
struct ReturnsTokenIdentifierTestInteract {
...
impl ReturnsTokenIdentifierTestInteract {
    async fn init() -> Self {
