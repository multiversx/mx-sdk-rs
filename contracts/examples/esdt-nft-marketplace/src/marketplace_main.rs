    ...
    storage::StorageModule + views::ViewsModule + events::EventsModule
{
    ...
    fn try_get_auction(&self, auction_id: u64) -> SCResult<Auction<Self::TypeManager>> {
        require!(
    ...
    fn calculate_winning_bid_split(
        &self,
        auction: &Auction<Self::TypeManager>,
    ) -> BidSplitAmounts<Self::TypeManager> {
    ...
    fn distribute_tokens_after_auction_end(&self, auction: &Auction<Self::TypeManager>) {
        let nft_type = &auction.auctioned_token.token_type;
        let nft_nonce = auction.auctioned_token.nonce;
        ...
        if auction.current_winner != Address::zero() {
            ...
            let nft_amount_to_send = match auction.auction_type {
                AuctionType::Nft | AuctionType::SftOnePerPayment => {
    ...
    fn try_set_bid_cut_percentage(&self, new_cut_percentage: u64) -> SCResult<()> {
        ...
        self.bid_cut_percentage()
            .set(&self.types().big_uint_from(new_cut_percentage));
