...
pub trait LinkedListMapperFeatures {
    ...
    fn list_mapper_pop_front(&self) -> Option<u32> {
        self.list_mapper().pop_front()
    }

    #[endpoint(listMapperPopBack)]
    fn list_mapper_pop_back(&self) -> Option<u32> {
        self.list_mapper().pop_back()
    }

    #[endpoint(listMapperFront)]
    fn list_mapper_front(&self) -> Option<u32> {
    ...
    fn list_mapper_back(&self) -> Option<u32> {
    ...
    fn list_mapper_push_after(&self, node_id: u32, element: u32) -> Option<u32> {
    ...
    fn list_mapper_push_before(&self, node_id: u32, element: u32) -> Option<u32> {
        let mut node_opt = self.list_mapper().get_node_by_id(node_id);
        if node_opt.is_none() {
            return None;
        }
    ...
    fn list_mapper_remove_node_by_id(&self, node_id: u32) -> Option<u32> {
        self.list_mapper().remove_node_by_id(node_id)
    }
}
