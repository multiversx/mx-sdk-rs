...
fn test_mxsc_pack() {
    let context = &InterpreterContext::default();
    assert_eq!(b"test".to_vec(), interpret_string("mxsc:tests/test.json", context));
...
fn test_mxsc_no_pack() {
    let context = &InterpreterContext::default();
    assert_eq!(b"not found :\"no_file.json\"".to_vec(), interpret_string("mxsc:no_file.json", context));
}
