...
"#;
    ...
    replace_with_error_message(&mut tx, String::new());
    assert_eq!(
        expected_tx.logs.unwrap().events[0].topics,
        tx.logs.unwrap().events[0].topics
    );
    ...
    let expected_tx_str = r#"
{
    ...
    replace_with_error_message(&mut tx, "Y2FsbGVyIGlzIG5vdCBhIGRlbGVnYXRvcg==".to_owned());
    assert_eq!(
        expected_tx.logs.unwrap().events[0].topics,
        tx.logs.unwrap().events[0].topics
    );
...
fn replace_logs_reason_sc_panic_test() {
    ...
    let expected_tx: TransactionOnNetwork =
        serde_json::from_str::<TransactionOnNetwork>(expected_tx_str).unwrap();
    replace_with_error_message(&mut tx, "c3RvcmFnZSBkZWNvZGUgZXJyb3IgKGtleTogcG9vbENvbnRyYWN0AAAAAAAAAe+/vSk6IGlucHV0IHRvbyBzaG9ydA==".to_owned());
    assert_eq!(
        expected_tx.logs.unwrap().events[0].topics,
        tx.logs.unwrap().events[0].topics
    );
...
"#;
    ...
    replace_with_error_message(
        &mut tx,
        "aW52YWxpZCBmdW5jdGlvbiAobm90IGZvdW5kKQ==".to_owned(),
    );
    assert_eq!(
        expected_tx.logs.unwrap().events[0].topics,
        tx.logs.unwrap().events[0].topics
