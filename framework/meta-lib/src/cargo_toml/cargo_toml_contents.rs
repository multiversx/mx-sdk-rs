...
impl CargoTomlContents {
    ...
    /// Returns `true` if this manifest contains a `[package]` section.
    ///
    /// Manifests without `[package]` are called *virtual manifests* — they only define
    /// a `[workspace]` and contain no crate of their own.
    pub fn has_package(&self) -> bool {
        self.toml_value.contains_key(PACKAGE)
    }
    ...
    pub fn add_workspace(&mut self, members: &[&str]) {
        ...
        // The resolver version is only needed in virtual workspaces (no [package] section).
        // When [package] is present, Cargo infers the resolver from the package's edition,
        // so omitting it here avoids a redundant field and Cargo does not warn.
        if !self.has_package() {
            workspace_table.insert(
                "resolver".to_string(),
                toml::Value::String(DEFAULT_WORKSPACE_RESOLVER.to_string()),
            );
        }
        workspace_table.insert("members".to_string(), members_toml);
