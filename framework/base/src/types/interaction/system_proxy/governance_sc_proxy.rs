use crate::types::{
    BigUint, ManagedAddress, ManagedBuffer, ManagedVec, NotPayable, ProxyArg, Tx, TxEnv, TxFrom,
    ...
    Gas: TxGas<Env>,
{
    ...
    pub fn clear_ended_proposal(
        self,
        voters: &ManagedVec<Env::Api, ManagedAddress<Env::Api>>,
    ) -> TxTypedCall<Env, From, To, NotPayable, Gas, ()> {
        let mut tx = self
            .wrapped_tx
            .payment(NotPayable)
