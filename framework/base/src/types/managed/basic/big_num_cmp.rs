...
pub(crate) fn cmp_i64<'a, M, B>(bi: &B, other: i64) -> Ordering
where
    M: ManagedTypeApi<'a>,
    B: ManagedType<'a, M, OwnHandle = M::BigIntHandle>,
{
    ...
    if other == 0 {
        ...
        let big_int_temp_1 = BigInt::<'a, M>::make_temp(const_handles::BIG_INT_TEMPORARY_1, other);
        api.bi_cmp(bi.get_handle(), big_int_temp_1)
    }
}

pub(crate) fn cmp_conv_i64<'a, M, B, T>(bi: &B, other: T) -> Ordering
where
    M: ManagedTypeApi<'a>,
    B: ManagedType<'a, M, OwnHandle = M::BigIntHandle>,
    T: TryInto<i64>,
{
    cmp_i64(bi, cast_to_i64::<'a, M, _>(other))
}
