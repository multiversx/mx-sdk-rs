...
where
    A: CallValueApi<'a>  + ErrorApi + ManagedTypeApi<'a>,
{
    ...
    A: CallValueApi<'a>  + ErrorApi + ManagedTypeApi<'a>,
{
    ...
    pub fn egld_value(&self) -> ManagedRef<'static, A, BigUint<'a, A>> {
    ...
    pub fn all_esdt_transfers(&self) -> ManagedRef<'static, A, ManagedVec<'a, A, EsdtTokenPayment<'a, A>>> {
    ...
    pub fn multi_esdt<const N: usize>(&self) -> [EsdtTokenPayment<'a, A>; N] {
    ...
    pub fn single_esdt(&self) -> EsdtTokenPayment<'a, A> {
    ...
    ///
    /// The amount cannot be 0, since that would not qualify as an ESDT transfer.
    pub fn single_fungible_esdt(&self) -> (TokenIdentifier<'a, A>, BigUint<'a, A>) {
    ...
    pub fn egld_or_single_esdt(&self) -> EgldOrEsdtTokenPayment<'a, A> {
    ...
    ///
    /// In case no transfer of value happen, it will return a payment of 0 EGLD.
    pub fn egld_or_single_fungible_esdt(&self) -> (EgldOrEsdtTokenIdentifier<'a, A>, BigUint<'a, A>) {
    ...
    pub fn any_payment(&self) -> EgldOrMultiEsdtPayment<'a, A> {
        let esdt_transfers = self.all_esdt_transfers();
