...
fn call_value_init_snippet(mpm: MethodPayableMetadata) -> proc_macro2::TokenStream {
    match &mpm {
        MethodPayableMetadata::NotPayable => {
            quote! {
                mx_sc::io::call_value_init::not_payable::<Self::Api>();
            }
        },
        MethodPayableMetadata::Egld => {
            quote! {
                mx_sc::io::call_value_init::payable_egld::<Self::Api>();
            }
        },
        MethodPayableMetadata::SingleEsdtToken(token_identifier) => {
            quote! {
                mx_sc::io::call_value_init::payable_single_specific_token::<Self::Api>(#token_identifier);
            }
        },
        MethodPayableMetadata::AnyToken => {
            quote! {
                mx_sc::io::call_value_init::payable_any::<Self::Api>();
            }
        },
    }
}
...
fn opt_payment_arg_snippet(
    ...
    opt_arg
        ...
        .map(|arg| {
            let pat = &arg.pat;
            quote! {
                let #pat = mx_sc::io::call_value_init::#init_fn_name::<Self::Api>();
            }
        })
        .unwrap_or_default()
}
