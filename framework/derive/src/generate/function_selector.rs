...
pub fn generate_function_selector_body(contract: &ContractTrait) -> proc_macro2::TokenStream {
    let match_arms: Vec<proc_macro2::TokenStream> = contract
        ...
        .filter_map(|m| match &m.public_role {
            PublicRole::Init(_) => Some(endpoint_match_arm(
                m,
                "init",
                quote! { if !<CurrentApi as multiversx_sc::api::VMApi>::external_view_init_override() },
            )),
    ...
    quote! {
        if match fn_name {
            ...
            "init" if <CurrentApi as multiversx_sc::api::VMApi>::external_view_init_override() => {
                multiversx_sc::external_view_contract::external_view_contract_constructor::<CurrentApi>();
                return true;
            },
            #(#match_arms)*
            other => false
