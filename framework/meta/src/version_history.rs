...
pub fn template_versions(start: usize) -> &'static [&'static str] {
    &VERSIONS[start..]
}

pub fn validate_template_tag(start: usize, tag: &str) -> bool {
    let versions = template_versions(start);
...
pub mod tests {
    ...
    fn template_versions_test() {
        assert_eq!(template_versions(33)[0], "0.43.0");

        assert!(validate_template_tag(33, "0.43.0"));
        assert!(!validate_template_tag(33, "0.42.0"));
    }

    #[test]
    fn template_versions_with_autogenerated_wasm() {
        assert_eq!(template_versions(40)[0], "0.45.0");

        assert!(validate_template_tag(40, "0.45.0"));
        assert!(!validate_template_tag(40, "0.44.0"));
    }

    #[test]
    fn template_versions_with_autogenerated_multiversx_json() {
        assert_eq!(template_versions(39)[0], "0.44.0");

        assert!(validate_template_tag(39, "0.44.0"));
        assert!(!validate_template_tag(39, "0.43.0"));
    }
}
