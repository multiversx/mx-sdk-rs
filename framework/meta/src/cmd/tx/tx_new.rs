...
async fn tx_new_inner(args: &NewArgs) -> Result<()> {
    ...
    if args.tx.send {
        ...
        let proxy = &args.gateway.proxy;
        let tx_hash = broadcast_transaction(&client, proxy, &output.emitted_transaction).await?;
        ...
        if args.tx.wait_result {
            ...
            let result = fetch_tx_on_network(proxy, &tx_hash).await?;
