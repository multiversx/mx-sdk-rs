...
use super::proxy_process_type_name::{
    extract_paths, extract_struct_crate, process_rust_type, proxy_methods_type_name, proxy_type_name
};
...
where
    ...
    fn extract_path_rename(&self) -> (&str, &str) {
        if let Some(path_rename) = &self.proxy_config.path_rename {
            return (&path_rename.from, &path_rename.to);
        }
    ...
    fn process_paths(&self, paths: &Vec<String>) -> Vec<String> {
        ...
        let (path_rename_from, path_rename_to) = self.extract_path_rename();
        let crate_name = self
            .meta_config
            .original_contract_abi
            .get_crate_name_for_code();
        ...
        for path in paths {
            ...
            let processed_path = match path {
                p if p == path_rename_from => path_rename_to.to_string(),
                p if crate_name == extract_struct_crate(p)
                    || TYPES_FROM_FRAMEWORK.contains(&type_rust_name) =>
                {
                    type_rust_name.to_string()
                },
                _ => path.to_string(),
            };
