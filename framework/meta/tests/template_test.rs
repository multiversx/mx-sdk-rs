...
fn test_template_list() {
    let workspace_path = find_workspace(&std::env::current_dir().unwrap()).unwrap();
...
pub fn cargo_test(target: &ContractCreatorTarget) {
    let workspace_target_dir = find_workspace(&std::env::current_dir().unwrap()).unwrap().join("target");
