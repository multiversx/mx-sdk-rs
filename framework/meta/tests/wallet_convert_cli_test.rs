...
fn test_convert_mnemonic_to_pem_file_in_file_out() {
    ...
    assert_eq!(wallet.private_key_hex().unwrap(), KNOWN_PRIVATE_KEY);
    assert_eq!(wallet.public_key().unwrap().to_hex(), KNOWN_PUBLIC_KEY);
...
fn test_convert_mnemonic_to_pem_custom_hrp() {
    ...
    let wallet = Wallet::from_pem_file(&output_pem).unwrap();
    assert_eq!(wallet.private_key_hex().unwrap(), KNOWN_PRIVATE_KEY);
    assert_eq!(wallet.public_key_hex().unwrap(), KNOWN_PUBLIC_KEY);
