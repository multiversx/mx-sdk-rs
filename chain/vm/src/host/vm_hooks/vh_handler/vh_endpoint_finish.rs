...
pub trait VMHooksEndpointFinish: VMHooksHandlerSource + VMHooksManagedTypes {
    fn finish_slice_u8(&mut self, slice: &[u8]) -> Result<(), VMHooksEarlyExit> {
    ...
    fn finish_big_int_raw(&mut self, handle: RawHandle) -> Result<(), VMHooksEarlyExit> {
    ...
    fn finish_big_uint_raw(&mut self, handle: RawHandle) -> Result<(), VMHooksEarlyExit> {
    ...
    fn finish_managed_buffer_raw(&mut self, handle: RawHandle) -> Result<(), VMHooksEarlyExit> {
        self.use_gas(
            self.gas_schedule()
    ...
    fn finish_i64(&mut self, value: i64) -> Result<(), VMHooksEarlyExit> {
        if value == 0 {
    ...
    fn finish_u64(&mut self, value: u64) -> Result<(), VMHooksEarlyExit> {
        if value == 0 {
            self.finish_slice_u8(&[])
        } else {
